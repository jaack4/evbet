...
def display_nfl_stats(min_ev=None, min_mean_diff=None):
    ...
    if filters:
        print(f"NFL BETTING PERFORMANCE REPORT ({', '.join(filters)})")
    else:
    ...
    with Database() as db:
        ...
        print("-"*60)
        stats = db.get_nfl_win_loss_stats(min_ev=min_ev, min_mean_diff=min_mean_diff)
        ...
        if stats and stats['total_bets'] > 0:
            ...
            print("No graded NFL bets found matching the specified filters.")
            return
        ...
        print("-"*60)
        bookmaker_stats = db.get_nfl_win_loss_by_bookmaker(min_ev=min_ev, min_mean_diff=min_mean_diff)
        ...
        print("-"*60)
        market_stats = db.get_nfl_win_loss_by_market(min_ev=min_ev, min_mean_diff=min_mean_diff)
...
if __name__ == "__main__":
    ...
    display_nfl_stats(min_ev=args.min_ev, min_mean_diff=args.min_mean_diff)
