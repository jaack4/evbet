#!/usr/bin/env python3
"""
Unified script to display betting statistics for NFL, NBA, or both sports
"""
import os
import argparse
...
def display_sport_stats(db, sport, min_ev=None, min_mean_diff=None):
    """Display statistics for a specific sport
    ...
    """
    get_stats_func = db.get_nfl_win_loss_stats if sport == 'NFL' else db.get_nba_win_loss_stats
    get_bookmaker_func = db.get_nfl_win_loss_by_bookmaker if sport == 'NFL' else db.get_nba_win_loss_by_bookmaker
    get_market_func = db.get_nfl_win_loss_by_market if sport == 'NFL' else db.get_nba_win_loss_by_market
    
    print(f"\nOVERALL {sport} STATISTICS")
    print("-"*60)
    stats = get_stats_func(min_ev=min_ev, min_mean_diff=min_mean_diff)
    ...
    if stats and stats['total_bets'] > 0:
        ...
        print(f"No graded {sport} bets found matching the specified filters.")
        return False
    
    # Stats by bookmaker
    print("\n" + "-"*60)
    print(f"PERFORMANCE BY BOOKMAKER ({sport})")
    print("-"*60)
    bookmaker_stats = get_bookmaker_func(min_ev=min_ev, min_mean_diff=min_mean_diff)
    ...
    # Stats by market
    print("\n" + "-"*60)
    print(f"PERFORMANCE BY MARKET TYPE ({sport})")
    print("-"*60)
    market_stats = get_market_func(min_ev=min_ev, min_mean_diff=min_mean_diff)
...
def display_stats(sport='both', min_ev=None, min_mean_diff=None):
    """Display betting statistics
    
    Args:
        sport (str): 'nfl', 'nba', or 'both'
        min_ev (float, optional): Minimum EV percentage to filter by
        min_mean_diff (float, optional): Minimum absolute mean difference to filter by
    """
    sport = sport.lower()
    ...
    if sport == 'both':
        display_combined_stats(min_ev=min_ev, min_mean_diff=min_mean_diff)
    else:
        # Single sport view
        ...
        if filters:
            print(f"{sport_upper} BETTING PERFORMANCE REPORT ({', '.join(filters)})")
        else:
            print(f"{sport_upper} BETTING PERFORMANCE REPORT (ALL BETS)")
        print("="*60 + "\n")
        
        with Database() as db:
            has_data = display_sport_stats(db, sport_upper, min_ev=min_ev, min_mean_diff=min_mean_diff)
            if has_data:
                print("\n" + "="*60 + "\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Display betting performance statistics',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Show combined stats for both sports
  python stats_report.py
  
  # Show NFL stats only
  python stats_report.py --sport nfl
  
  # Show NBA stats only
  python stats_report.py --sport nba
  
  # Filter by minimum EV
  python stats_report.py --min-ev 5
  
  # Filter by minimum mean difference (line separation)
  python stats_report.py --min-mean-diff 0.5
  
  # Combine filters for NFL only
  python stats_report.py --sport nfl --min-ev 3 --min-mean-diff 0.5
        """
    )
    parser.add_argument(
        '--sport',
        ...
        help='Which sport to display stats for (default: both)'
    )
    parser.add_argument(
        '--min-ev',
        type=float,
    ...
    display_stats(sport=args.sport, min_ev=args.min_ev, min_mean_diff=args.min_mean_diff)
